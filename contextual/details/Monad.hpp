...
namespace Contextual::Details {
  ...
  template<typename Context>
  concept HasMinimalMonad = (HasFlatMap<Context> && HasPure<Context>) ||
                            (HasFMap<Context> && HasPure<Context> &&
                             HasFlatten<Context>);
  ...
  template<typename Context>
  concept HasMonadCore = HasFunctorCore<Context> &&
    HasApplicativeCore<Context> && HasFlatMap<Context> && HasFlatten<Context>;

  template<typename Context>
  concept MissingMonadCore = !HasMonadCore<Context>;

  template<typename Context>
  concept HasReturnM = requires
  {
    {Context::returnM};
  };
  ...
  concept HasMonadUtility =
    HasApplicativeUtility<Context> && HasMonadCore<Context> &&
    HasReturnM<Context> && HasLetM<Context> && HasBeginM<Context>;
    ...
    class MixinBeginM : public Static_curried<MixinBeginM, Nat<1>>
    {
      ...
      class Result : public Base
      {
        ...
        public:
          ...
          call(T&& mx, Ts&&... mxs)
          {
            if constexpr (count_types<Ts...>() == 0) {
              return mx;
            } else {
              return Base::flatMap([=](auto) { return call(mxs...); }, mx);
            }
          }
        };
  ...
  class ProtoMonad : public Applicative
  {
    ...
    public:
      ...
      {
        return letC(askExpel, [=](auto expel) {
          return letC(askFlatten, [=](auto flatten) {
            return letC(fMap(expel, digest(cmcmx)), [=](auto mmx) {
              return returnC(flatten(mmx));
            });
          });
        });
      }
