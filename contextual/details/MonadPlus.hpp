...
namespace Contextual::Details
{
  ...
  concept HasMZero = requires{{Context::mZero};};

  template<typename Context>
  concept MissingMZero = ! HasMZero<Context>;

  template<typename Context>
  concept HasMPlus = requires{{Context::mPlus};};

  template<typename Context>
  concept MissingMPlus = ! HasMPlus<Context>;

  template<typename Context>
  concept HasMinimalMonadPlus =
    HasMinimalMonad<Context> &&
    (HasFail<Context> || HasMZero<Context>) &&
    HasMPlus<Context>;

  template<typename Context>
  concept HasMonadPlusCore =
    HasMonadCore<Context> &&
    HasMonadFailCore<Context> &&
    HasMZero<Context> &&
    HasMPlus<Context>;
  ...
  class MonadPlus : public Derive<ProtoMonadPlus,MixinMonadPlus, MixinMonadPlusUtility>
  {};
