...
namespace Contextual::Details
{
  ...
  concept HasMEmpty = requires{{Context::mEmpty};};

  template<typename Context>
  concept MissingMEmpty = ! HasMEmpty<Context>;

  template<typename Context>
  concept HasMAppend = requires{{Context::mAppend};};

  template<typename Context>
  concept MissingMAppend = ! HasMAppend<Context>;

  template<typename Context>
  concept HasMAppend_ = requires{{Context::mAppend_};};
  ...
  class MixinMonoid : public Static_curried<MixinMonoid, Nat<1>>{
    ...
    //              |_|  |_|
    class MixinMAppendFromMAppend_ : public Static_curried<MixinMAppendFromMAppend_, Nat<1>>{
    ...
    class MixinMAppendFromOp_ : public Static_curried<MixinMAppendFromOp_, Nat<1>>{
    ...
    class MixinMAppend_ : public Static_curried<MixinMAppend_, Nat<1>>{
      ...
      class Result : public Base {
        ...
        public:
          static constexpr auto
          call(){ return Base::mEmpty; }

          template<typename T, typename U>
          static constexpr auto
          call(T&& x, U&& y){ return Base::mAppend(forward<T>(x), forward<U>(y)); }

          template<typename T, typename U, typename V, typename ... Vs>
          static constexpr auto
          call(T&& x, U&& y, V&& z, Vs&& ... zs){
            return call(Base::mAppend(forward<T>(x), forward<T>(y)),
                        forward<V>(z),
                        forward<Vs>(zs) ...);
          }
        };
